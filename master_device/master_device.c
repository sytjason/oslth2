...
////////////////////////////////////////////////////
void easy_mmap_open(struct vm_area_struct *vma)
{
}

void easy_mmap_close(struct vm_area_struct *vma)
{
}

static int easy_mmap_fault(struct vm_area_struct *vma, struct vm_fault *vmf)
{
	printk("master_fault\n");
...
static const struct vm_operations_struct easy_vm_ops =
{
	.open 	= easy_mmap_open,
	.close	= easy_mmap_close,
	.fault	= easy_mmap_fault
};

static int easy_mmap(struct file *file, struct vm_area_struct *vma)
{
	//unsigned long addr;
	//addr = vir_to_phys(file->private_data) >> PAGE_SHIFT;
...
static long master_ioctl(struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
{
	...
	switch(ioctl_num){
		...
		///////////////////////////////////////////////////////////////
			nsend = ksend(sockfd_cli, file->private_data, ioctl_param, 0);
			//nsend = send_msg(file,file->private_data,ioctl_param,NULL);
			// while (offset < ioctl_param)
			// {
			// 	//size_t length = PAGE_SIZE;
			// 	size_t length = 512;
			// 	if ((ioctl_param - offset) < length)
			// 		length = ioctl_param - offset;
			// 	nsend = ksend(sockfd_cli, file->private_data+offset, length, 0);
			// 	if (nsend < 0)
			// 		break;
			// 	offset += nsend;
			// 	printk("master_IOCTL_MMAP nsend = %d\n", nsend);
			// }
 			printk("master_IOCTL_MMAP ioctl_param = %d offset = %d  nsend = %d %s\n", ioctl_param, offset, nsend,file->private_data);
			// printk("ioctl_param =%d\n", ioctl_param);
			// ret = ksend(sockfd_cli, file->private_data, ioctl_param, 0);
			// printk("master_IOCTL_MMAP ret=%d\n", ret);
			break;
		///////////////////////////////////////////////////////////////
		case master_IOCTL_EXIT:
			printk("master closed\n");
